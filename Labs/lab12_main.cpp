...
GLUquadricObj* qobj_noTM = gluNewQuadric();
GLint pos;
...
void draw()
{
	...
	if(pos==0) draw_string(GLUT_BITMAP_TIMES_ROMAN_24, "Light: Eye", -4.7, 4.5, 0, 1, 1);
	else if (pos == 1) draw_string(GLUT_BITMAP_TIMES_ROMAN_24, "Light: Cube", -4.7, 4.5, 0, 1, 1);
	else if (pos == 2) draw_string(GLUT_BITMAP_TIMES_ROMAN_24, "Light: Cider", -4.7, 4.5, 0, 1, 1);
	else if (pos == 3) draw_string(GLUT_BITMAP_TIMES_ROMAN_24, "Light: Earth", -4.7, 4.5, 0, 1, 1);
	else  draw_string(GLUT_BITMAP_TIMES_ROMAN_24, "Light: (10, 5, 3)", -4.7, 4.5, 0, 1, 1);
	...
	gluQuadricDrawStyle(qobj_noTM, GLU_LINE);
	gluQuadricNormals(qobj_noTM, GLU_SMOOTH);

	glPushMatrix();
	glColor3f(1, 1, 1);
	glTranslatef(1, 0, 5.9);
	gluQuadricTexture(qobj_noTM, GL_TRUE);
	glBindTexture(GL_TEXTURE_2D, tex_cider[0]);
	gluPartialDisk(qobj_noTM, 0.5, 2.5, 60, 30, 90, 270);
	glPopMatrix();
...
void draw_cider(GLfloat cider_x, GLfloat cider_y, GLfloat cider_z)
{
	...
	gluCylinder(qobj, 1.0f, 0.0f, 4.0f, 16, 100);
	...
	gluDisk(qobj, 0.0f, 1.0f, 100, 100);
	glPopMatrix();
}
...
void setting_quadric()
{
	...
	gluQuadricDrawStyle(qobj, GLU_LINE); // Solid 형태로 객체를 그린다.
